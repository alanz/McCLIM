...
(defun generate-ratio (&key bound)
  (let ((gen (gen-float :bound bound)))
    (lambda () (rationalize (funcall gen)))))
...
(defun generate-region (&optional (types +composable-types+))
  (let ((gen-type (apply #'gen-one-element types))
        (gen-len (gen-integer :min 4 :max 12))
    ...
    (lambda ()
      (funcall
       (ecase (funcall gen-type)
         ...
         (standard-polygon (generate-polygon  5))
...
(test algebra.inclusion.transitivity
  (for-all* ((r1 (generate-region))
             (r2 (generate-region))
             (r3 (generate-region)))
    (when (and (and (region-contains-region-p r2 r1)
...
(test algebra.inclusion.existance-meets
  (for-all* ((r1 (generate-region))
    ...
    (when (and (and (region-contains-region-p r1 r3)
